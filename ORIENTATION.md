...
| `FRESH-INSTALL.bat` | `scripts\fresh_install.ps1` | 3 | Reinstall from scratch on the air-gapped PC, run from the kit on the stick: stops VMD, moves the old `C:\VMD` aside to `C:\VMD-old-<date>` (kept, not deleted), copies the kit in, puts back `settings.json`, `detection.json` and `cameras\`, then runs the offline installer. Green DONE / red FAILED. |
